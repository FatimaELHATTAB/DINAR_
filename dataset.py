...
def split_target_shadow_dataset(dataset, args, shadow_split=0.5, ):
    ...
    # t_train_loader = torch.utils.data.DataLoader(dataset=t_train_data,
    #                                      batch_size=args.local_bs,
    #                                      collate_fn=celeba_collate_fn,
    #                                      num_workers=num_workers)
    #
    # images_set, labels_set = [], []
    # for _, (images, labels) in enumerate(t_train_loader):
    #     images_set.append(images)
    #     labels_set.append(labels)
    #
    # t_train_labels = torch.cat(labels_set).cpu().detach().numpy()
    # t_train_images = torch.cat(images_set).cpu().detach().numpy()
    #
    # np.save('/home2/cboscher/code/pastel2.0/model_transfer_exp/celebA_t_train_labels.npy', t_train_labels)
    # np.save('/home2/cboscher/code/pastel2.0/model_transfer_exp/celebA_t_train_images.npy', t_train_images)
    #
    # t_test_loader = torch.utils.data.DataLoader(dataset=t_out_data,
    #                                      batch_size=args.local_bs,
    #                                      collate_fn=celeba_collate_fn,
    #                                      num_workers=num_workers)
    #
    # images_set, labels_set = [], []
    # for _, (images, labels) in enumerate(t_test_loader):
    #     images_set.append(images)
    #     labels_set.append(labels)
    #
    # t_out_labels = torch.cat(labels_set).cpu().detach().numpy()
    # t_out_images = torch.cat(images_set).cpu().detach().numpy()
    # np.save('/home2/cboscher/code/pastel2.0/model_transfer_exp/celebA_t_test_labels.npy', t_out_labels )
    # np.save('/home2/cboscher/code/pastel2.0/model_transfer_exp/celebA_t_test_images.npy',t_out_images )
    #
    # s_train_loader = torch.utils.data.DataLoader(dataset=s_train_data,
    #                                              batch_size=args.local_bs,
    #                                              collate_fn=celeba_collate_fn,
    #                                              num_workers=num_workers)
    #
    # images_set, labels_set = [], []
    # for _, (images, labels) in enumerate(s_train_loader):
    #     images_set.append(images)
    #     labels_set.append(labels)
    #
    # s_train_labels = torch.cat(labels_set).cpu().detach().numpy()
    # s_train_images = torch.cat(images_set).cpu().detach().numpy()
    # np.save('/home2/cboscher/code/pastel2.0/model_transfer_exp/celebA_s_train_labels.npy', s_train_labels)
    # np.save('/home2/cboscher/code/pastel2.0/model_transfer_exp/celebA_s_train_images.npy',  s_train_images)
    #
    # s_test_loader = torch.utils.data.DataLoader(dataset=s_out_data,
    #                                              batch_size=args.local_bs,
    #                                              collate_fn=celeba_collate_fn,
    #                                              num_workers=num_workers)
    #
    # images_set, labels_set = [], []
    # for _, (images, labels) in enumerate(s_test_loader):
    #     images_set.append(images)
    #     labels_set.append(labels)
    #
    # s_out_labels = torch.cat(labels_set).cpu().detach().numpy()
    # s_out_images = torch.cat(images_set).cpu().detach().numpy()
    # np.save('/home2/cboscher/code/pastel2.0/model_transfer_exp/celebA_s_test_labels.npy', s_out_labels)
    # np.save('/home2/cboscher/code/pastel2.0/model_transfer_exp/celebA_s_test_images.npy',  s_out_images)
